...
PATH_OBSTACLES = {
    ...
    "bounce-simplified": ["B2", "B10", "D3", "D5", "D7", "D8"],
}
...
def anim_traj(
    ...
    if goal is not None:
        ax.add_collection(
            mpl.collections.LineCollection([goal], color="r", linewidths=4,)
        )
...
def plot_traj(
    ...
    if invert:
        plt.gca().invert_xaxis()
        plt.gca().invert_yaxis()
    plt.savefig('{}.png'.format(title), transparent=True, dpi=800)
...
def index_trajectories():
    from constants import Trajectory, OptiParams
    trajectories = {}
